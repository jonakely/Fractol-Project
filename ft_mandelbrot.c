...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/18 14:46:38 by jonandri          #+#    #+#             */
/*   Updated: 2025/08/21 10:50:40 by jonandri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_variable_to_mandelbrot(t_v *v, t_data *vars)
{
	v->c_im = ft_im(vars, v->y);
	v->c_re = ft_re(vars, v->x);
	v->z_im = 0;
	v->z_re = 0;
	v->i = 0;
}
...
void	ft_mandelbrot(t_data *vars)
{
	...
	while ((v.y++) < vars->y)
	{
		...
		while ((v.x++) < vars->x)
		{
			ft_variable_to_mandelbrot(&v, vars);
			while (v.i < 200)
			{
				if (ft_complex(&v) > 4)
					break ;
			...
			my_mlx_pixel_put(vars, v.x, v.y, (ft_color(v.i, 98)));
		}
	}
